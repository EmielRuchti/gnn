...
sweep_config = {
    "name": "gcn-test-normalize-runs-10-submodel-11",
    "method": "grid",
    "parameters": {
        "hidden_channels": {
            "value": 128
...
"""
class GCN_skip_2_layer_linear_in_out_8(torch.nn.Module):
...
GCN Skip 2 layer linear_in_out default, every layer, plus bns, relu, dropout
"""
class GCN_skip_2_layer_linear_in_out_11(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout, num_layers, skip_coefficient):
        super().__init__()
        #torch.manual_seed(1234567)
        ...
        for i in range(1,(len(self.convs)-1)):
            ...
            if i%2 == 0:
                if i == 2:
                    ...
                    x = x + x_start + (self.skip_coefficient * out[i-3])
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
            out.append(x)
        x = sum(out)
...
def runArxivNormalize():
    ...
    data = data.to(device)
    print("Model 6")
    model = GCN_skip_2_layer_linear_in_out_11(data.num_features, wandb.config.hidden_channels, ogbndataset.num_classes,
                                             wandb.config.dropout, wandb.config.num_layers,
                                             wandb.config.skip_coefficient).to(device)
    logger = Logger(10)
...
wandb.agent(sweep_id_arxiv, project="GCN_testing_Arxiv", function=runArxivNormalize)
